...
def supervisor_packet(target_root: Path, run_dir: Path, mutation: dict[str, Any], assignments: list[dict[str, Any]], next_command: str) -> str:
    ...
    lines = [
        "# Supervisor Packet",
        "",
        "Start the run by drafting the improvement plan before any worker executes anything.",
        "Do not dispatch workers until the human has reviewed the plan, rubric, and proposed mutation.",
        "",
        "## Your Job",
        "",
        ...
        "- if the target skill is chat-oriented, design the default test shape as a multi-turn conversation",
        "",
        "## Files To Fill",
        "",
        ...
        *[f"- {worker}" for worker in workers],
        ...
        "- get human feedback on the plan and rubric before worker execution starts",
        "- for chat skills, avoid single-turn-only tests unless the behavior under test is truly single-turn",
        "",
        "## Next Command",
        "",
        f"`{next_command}`",
...
def worker_packet(run_dir: Path, worker_assignment: dict[str, Any], baseline: dict[str, Any], mutation: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- if the case is multi-turn, record the full turn sequence for both baseline and mutation",
            "- preserve the full agent-to-agent conversation history between the skill-using agent and the worker context so a reviewer can reconstruct the conversation later",
            "- do not replace the raw history with only a cleaned summary or final answer excerpt",
...
def evaluator_packet(run_dir: Path, worker_assignment: dict[str, Any], mutation: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- for chat skills, judge turn-by-turn behavior rather than only the first reply",
            "- for multi-turn chat skills, use the preserved conversation history as primary evidence rather than only the final turn",
        ]
    )
    return "\n".join(lines) + "\n"
...
def coordinator_packet(target_root: Path, run_dir: Path, run_id: str, assignments: list[dict[str, Any]], final_summary: str) -> str:
    ...
    lines = [
        ...
        "## Sequence",
        "",
        "1. Complete the supervisor draft first: plan, improvement target, rubric, and one bounded mutation.",
        "2. Show that draft to the human and collect feedback before any worker executes.",
        "3. Dispatch the worker packets so each worker executes baseline and mutation on its shard.",
        "4. Run the review command to generate deterministic evidence and refresh evaluator packets.",
        "5. Dispatch evaluator packets and wait for all `judgments/*.json` files to be completed.",
        "6. Finalize the run with the command below.",
        "",
        "## Commands",
        "",
